...
from qstyles.palettes import Base, Categorical, Diverging, Sequential
...
def create_palette(palette: str) -> cycler:
    ...
    diverging_palettes = ["qs_plum_green", "qs_green_violet"]
    base_palettes = ["grey_dark", "grey_light"]
    ...
    elif palette in base_palettes:
        hex_values = getattr(Base, palette)

    return cycler("color", hex_values)


def set_theme(theme_params: dict) -> None:
    """Pass a dictionary of style parameters to update matplotlib.rcParams.
...
def compose_theme(
    ...
    Args:
        ...
        theme_dict (dict): Dictionary containing matplotlib style params,
            this is automatically generated by the qstyles package.
    ...
    if palette:
        ...
        # make violet the default palette
        palette = f"qs_violet_{style}"
    ...
    # add colour cycler and context to the theme dictionary
    theme_dict.update(custom_cycler)
    theme_dict.update(plotting_context(context))

    # set grid on or off based on the grid parameter
    if grid:
        theme_dict.update(
            {
                "axes.grid": True,
